...
//
// The genuinely-algorithmic, DOM-free cores live in sibling ES modules under
// src/ and are imported below; they are unit-tested under test/. This file
// keeps only rendering, control wiring and animation.
// ============================================================

import { traceSwap, traceSumDigits, tracePow } from './trace.js';
import { tokenize, parse, evalNode } from './expr.js';
import { runLoop } from './loops.js';
import { factorial, fib, gcd } from './recursion.js';
import { linearSearch, binarySearch } from './search.js';
import { SORTS } from './sorting.js';
import { encode, bitsToUnsigned, range, bitOp } from './bits.js';
import { layout as structLayout, TYPE_SIZE } from './structs.js';
import { safe as queenSafe } from './queens.js';
...
(function tracing() {
  ...
  // The stepping logic lives in ./trace.js (tested); this only pairs it with
  // the displayed source lines.
  const PROGRAMS = {
    swap: {
...
(function expressions() {
  ...
  // tokenize / parse / evalNode are imported from ./expr.js (tested).
...
(function loops() {
  ...
  function compute() {
    // runLoop (./loops.js, tested) does the accumulation.
    return runLoop(n('lp-a', 1), n('lp-n', 6), n('lp-s', 1), $('lp-kind').value);
  }
  function draw() {
    const { ctx, w, h } = fitCanvas(cv);
    ctx.clearRect(0, 0, w, h);
...
(function recursion() {
  ...
  let depth;
  // factorial / fib / gcd, their call counts and the call tree come from
  // ./recursion.js (tested). The tree nodes carry {label, value, children}.
  ...
  function draw() {
    ...
    depth = 0;
    let result;
    if (fn === 'fact') result = factorial(N);
    else if (fn === 'fib') result = fib(N);
    else result = gcd(48, ([0, 1, 2, 3, 5, 8, 13, 21][N] || N)); // varied second arg
    const root = result.tree, calls = result.calls;
    const box = { x: 0 };
    place(root, 0, box);
...
(function bits() {
  ...
  // encode / bitsToUnsigned / range / bitOp come from ./bits.js (tested).
  function draw() {
    const { ctx, w, h } = fitCanvas(cv);
    ctx.clearRect(0, 0, w, h);
    const width = n('bt-w', 8), signed = $('bt-tc').checked;
    let val = Math.round(n('bt-v', 42));
    const { lo, hi } = range(width, signed);
    val = clamp(val, lo, hi);
    ...
    const LABELS = {
      and: `${u} & ${mask}`, or: `${u} | ${mask}`, xor: `${u} ^ ${mask}`,
      shl: `${u} << 1`, shr: `${u} >> 1`,
    };
    const res = op === 'none' ? null : bitOp(op, u, mask, width);
...
(function queens() {
  ...
  // queenSafe (./queens.js, tested) tests the current partial placement.
  const safe = (c, r) => queenSafe(cols, c, r);
...
(function structs() {
  ...
  const TYPES = TYPE_SIZE; // { char:1, short:2, int:4, double:8 }
  const ORDERS = {
  ...
  // layout (alignment = type size; struct aligned to its largest member) comes
  // from ./structs.js (tested).
  function draw() {
    const { ctx, w, h } = fitCanvas(cv);
    ctx.clearRect(0, 0, w, h);
    const fields = ORDERS[$('st-order').value];
    const { cells, size, dataBytes, padding: pad } = structLayout(fields);
